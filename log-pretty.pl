...
#
# Dependencies: files.list and function.list, generated by call-anal.pl
...
$x = readall("akbox.h");
#                                                                     1   1
($xd) = $x =~ /^\#define AKBOX_FN\(x\) \(\(int\)\(\( sizeof\(x\) \+ \(x\)\[0\] (.*?)\) & 0x0*ffff\)\)/ms;
die "akbox.h($x) xd($xd)" if !$xd;  # Something is amiss in the matching
$akbox_fn = 'sub akbox_fn { my ($x) = @_; return (length($x)+1+ord(substr($x,0,1))'."\n";
while ($xd =~ /\+ \(sizeof\(x\)>(\d+) \? (\d+)\*\(x\)\[(\d+)\] : 0\)/g) {
    warn "siz($1) factor($2) tap($3) pos=".pos($xd) if $debug;
    warn "mismatch in siz($1) and tap($3)" if $1-1 != $3;
    $akbox_fn .= "+ $2*ord(substr(\$x,$3,1))\n";
}
$akbox_fn .= ') & 0x0000ffff; }';
warn $akbox_fn if $debug;
eval $akbox_fn;
...
while ($x =~ /ZXFILE_DEF\("([A-Za-z_0-9.-]+)"\)/g) {
    ...
    if ($files{$k}) {
	warn "hash collision for($k)=($files{$k})" if $debug;
	$fn .= ",$files{$k}";
    }
    $files{$k} = $fn;
}
...
while ($x =~ /ZXFUNC_DEF\("([A-Za-z_0-9:]+)","([A-Za-z_0-9.-]+)"\)/g) {
    ...
    $k = sprintf("%04x",akbox_fn($fn));
    if ($funcs{$k}) {
	warn "hash collision for($k)=($funcs{$k})" if $debug;
	$fn .= ",$funcs{$k}";
    }
    $funcs{$k} = $fn;
    $func_files{$k} = $fl;    
