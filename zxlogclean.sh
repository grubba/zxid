...
-sym)    shift;  symkeyf=$1;  shift; passphrase="--passphrase-file $symkeyf"; continue;;
-symfd)  shift;  symkeyfd=$1; shift; passphrase="--passphrase-fd $symkeyfd"; continue;;
-symstr) shift;  symkeys=$1;  shift; passphrase="--passphrase $symkeys"; continue;;
...
pgppub() {
    ...
    if cat $2 | xargs tar cf - | $PGP $zalgo $pubring $secring -r $pubkey -e $sig --batch > $1 ; then
...
pgpsym() {
    ...
    if xargs tar cf - < $2 | $PGP -$zalgo $pubring $secring $passphrase -c $sig --batch > $1 ; then
...
clean_inst() {
    ...
    echo log/xml.dbg > $backupdir/zxlogclean-todel-$$
    ...
    $FIND $dir/log \! -name .keep -atime +$trail_atime >$backupdir/zxlogclean-trail-$$
    ...
    $FIND $dir/ses -depth -atime +$ses_atime -type d >$backupdir/zxlogclean-ses-to-rm-$$
...
for inst in $* do
  if [ !-d $instance ] ; then warn "Instance($inst) is not a directory. Skipping."; continue; fi
  clean_inst $inst
done
