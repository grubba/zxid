...
static void hi_accept_book(struct hi_thr* hit, struct hi_io* io)
{
  ...
  if (n_thr) {
    ...
    NEVERNEVER("Should not be possible due to half close n_thr=%d", n_thr);
...
static struct hi_qel* hi_todo_consume(struct hiios* shf)
{
  ...
  D("LOCK todo_mut.thr=%x (going to cond_wait)", shf->todo_mut.thr);
...
void hi_todo_produce(struct hiios* shf, struct hi_qel* qe)
{
  ...
  if (!qe->intodo) {
    ...
    if (ONE_OF_2(qe->kind, HI_TCP_S, HI_TCP_C)) {
      io = ((struct hi_io*)qe);
      D("todo(%x) n_thr=%d reading=%d writing=%d ev=%x", io->fd, io->n_thr, io->reading, io->writing, io->events);
    } else {
...
static void hi_poll(struct hiios* shf)
{
  ...
  D("LOCK & UNLOCK todo_mut.thr=%x", shf->todo_mut.thr);
...
#define EPOLLIN  (POLLIN)
  ...
  if (io->events & EPOLLIN && !io->reading) {
    ...
    reading = io->reading = !io->cur_pdu || io->cur_pdu->need; /* only place where io->reading is set */
